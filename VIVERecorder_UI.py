...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Create a FigureCanvas and add it to the layout
        self.vr_manager = VRSystemManager()

        self.plot_3d = True  # live 3D plot (might affect performance)
        self.plot_t_xyz = False  # live plot of x, y, z positions
        self.log_data = True  # log data to CSV file

        # print(vr_manager.print_discovered_objects())
        if not self.vr_manager.initialize_vr_system():
            return
        readable_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
        if not self.csv_logger.init_csv(readable_time + '.csv'):
            return

        self.canvas = FigureCanvas(self.plotter.init_3d_plot())
        layout.addWidget(self.canvas)

        self.timer = QTimer()  # `self` is the parent, ensuring proper memory management
        self.timer.setInterval(10)  # Set interval to 1000 ms (1 second)
        ...
        for i in range(openvr.k_unMaxTrackedDeviceCount):
            if poses[i].bPoseIsValid:
                ...
                if device_class == openvr.TrackedDeviceClass_GenericTracker:
                    ...
                    if self.plot_3d:
                        self.plotter.update_3d_plot(position[:3])
